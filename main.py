...
MODELING_PROMPT_TEMPLATE = """你是资深投资策略师，请根据以下市场数据构建内部模型:
研究主题: {research_topic}
行业焦点: {industry_focus}
时间范围: {time_horizon}
...
REASONING_PROMPT_TEMPLATE = """你是战略投资顾问，请生成3个不同的投资方案:
研究主题: {research_topic}
行业焦点: {industry_focus}
时间范围: {time_horizon}
...
DECISION_PROMPT_TEMPLATE = """你是投资决策委员会主席，请评估候选方案并选择最优:
研究主题: {research_topic}
行业焦点: {industry_focus}
时间范围: {time_horizon}
...
REPORT_PROMPT_TEMPLATE = """你是专业投研报告撰写人，请生成完整投资报告:
研究主题: {research_topic}
行业焦点: {industry_focus}
时间范围: {time_horizon}
...
def perception_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if retry_count >= MAX_RETRIES:
        return {**state, "error": f"感知阶段重试超过{MAX_RETRIES}次，请检查API密钥和网络连接", "current_phase": "error"}
    try:
        # 检查 API Key
        if not state.get("api_key"):
            return {**state, "error": "API密钥未配置，请在侧边栏输入或设置环境变量 DASHSCOPE_API_KEY", "current_phase": "error", "perception_retry": MAX_RETRIES}
        ...
        error_msg = str(e)
        if "Authentication" in error_msg or "API key" in error_msg or "api.key" in error_msg:
            return {**state, "error": f"API密钥无效或已过期: {e}", "current_phase": "error", "perception_retry": MAX_RETRIES}
...
def modeling_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if retry_count >= MAX_RETRIES:
        return {**state, "error": f"建模阶段重试超过{MAX_RETRIES}次，请检查API密钥和网络连接", "current_phase": "error"}
    try:
        ...
        error_msg = str(e)
        if "Authentication" in error_msg or "API key" in error_msg or "api.key" in error_msg:
            return {**state, "error": f"API密钥无效或已过期: {e}", "current_phase": "error", "modeling_retry": MAX_RETRIES}
...
def report_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        chain = prompt | llm | StrOutputParser()
        result = chain.invoke({})
        return {**state, "final_report": result, "current_phase": "completed", "report_retry": 0, "error": None}
    except Exception as e:
...
# 优先从环境变量读取，兼容本地 .env 和 Streamlit Cloud Secrets
if 'api_key' not in st.session_state:
    st.session_state.api_key = os.getenv("DASHSCOPE_API_KEY", "")
if 'api_url' not in st.session_state:
    st.session_state.api_url = os.getenv("DASHSCOPE_API_KEY_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
if 'model_name' not in st.session_state:
    st.session_state.model_name = os.getenv("MODEL_NAME", "qwen-plus")
